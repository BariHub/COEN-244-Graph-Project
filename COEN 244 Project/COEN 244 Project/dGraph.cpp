...
dGraph::dGraph() : numNodes(0), numEdges(0)
{
...
bool dGraph::searchEdge(int a, int b)
{
	for (int i = 0; i < numEdges; i++) {
...
Node& dGraph::getNode(int ID) const
{
	for (int i = 0; i < numNodes; i++) {
		if (Nodes[i].getID() == ID) {
			return Nodes[i];
...
Edge& dGraph::getEdge(Node& first, Node& last) const
{
	for (int i = 0; i < numEdges; i++) {
...
int dGraph::getnumNodes() const
{
	return numNodes;
}
// gets number of edges
int dGraph::getnumEdges() const
{
	return numEdges;
}
// checks if it is empty - DONE
bool dGraph::isEmpty() const
{
...
void dGraph::printNodesByID() const
{
	for (int i = 0; i < numNodes; i++) {
		Nodes[i].print();
		cout << endl;
...
void dGraph::printPath(Node& n) const 
{
...
void dGraph::printAllPath() const
{
	for (int i = 0; i < numNodes; i++) {
